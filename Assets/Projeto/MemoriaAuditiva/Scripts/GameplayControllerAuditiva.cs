...
public class GameplayControllerAuditiva : MonoBehaviour
{
    //Imagem Padrão
    ...
    [field: Space] [field: Header("Sons")] public AudioSource AudioSource { get; private set; }
    ...
    [field: SerializeField] private List<CardControllerAuditiva> FlippedCards { get; set; } // Lista das cartas viradas
    private List<CardControllerAuditiva> MatchedCards { get; set; } // Lista das cartas combinadas

    private void Start()
    {
        Components();
        Shuffle(CardData);
        FlippedCards = new List<CardControllerAuditiva>();
        MatchedCards = new List<CardControllerAuditiva>();
        InitializeCards();
    }
    ...
    private void InitializeCards()
    {
        ...
        for (var x = 0; x < GridSizeX; x++)
        {
            for (var y = 0; y < GridSizeY; y++)
            {
                ...
                var card = cardObj.GetComponent<CardControllerAuditiva>();
    ...
    public void FlipCard(CardControllerAuditiva card)
    {
